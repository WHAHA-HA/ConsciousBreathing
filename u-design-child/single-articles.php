...
<?php           udesign_main_content_top( is_front_page() );
	if(get_bloginfo('language')=='sv-SE')
    	...
    	$lang_cat = "en";
    if($lang_cat=="en")
    	$category_slug = get_category_by_slug('articles');
    else
    	$category_slug = get_category_by_slug('artiklar');
    /* my_var_dump($category_slug);
    echo $category_slug->cat_name; */
?>
<?php		if (have_posts()) :
		    while (have_posts()) : the_post(); ?>
			<?php observePostViews(get_the_ID());?>
			<!-- <div><a href="http://www.consciousbreathing.com/articles"><span class="all_posts alignleft">ALL ARTICLES</span></a><span class="alignright"><?php next_post_link('%link', '<div class="next-button">Next</div>', TRUE, '15'); ?></span><span class="alignright"><?php previous_post_link('%link', '<div class="prev-button">Previous</div>', TRUE, '15'); ?></span></div>  -->
			<div class="testimonial_panel">
				...
				<?php else:?>
				<a href="<?php echo site_url($category_slug->slug);?>"><span
					class="all_posts alignleft">All Articles</span></a><!-- English text "ALL TESTIMONIALS" --><span
