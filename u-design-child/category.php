...
<?php       udesign_main_content_top( is_front_page() ); ?>
		<?php
			...
			query_posts('category_name='.$category[0]->slug.'&paged='.$paged);
		?>
	    <?php if (have_posts()) : ?>
...
<?php                               // Post Image
									if(has_post_thumbnail($post->ID))
									{
										...
										echo do_shortcode('[custom_frame_left]<a href="'.get_the_permalink().'">'.get_the_post_thumbnail($post->ID, array(150,150), array('class' => 'alignleft') ).'</a>[/custom_frame_left]');

									}
                                   ?>
                                   <img src="<?php echo $image[0];?>" width="150" height="80">
								</div>
...
<?php				    if ( $udesign_options['show_excerpt'] == 'yes' ) {
	...
	<?php
		if(get_bloginfo('language')=='sv_SE')
			echo do_shortcode('[custom_button text="Read more" url="'.get_the_permalink().'" size="medium" bg_color="#FF5C00" text_color="#FFFFFF" align="left" target="_self"]');
		else
			echo do_shortcode('[custom_button text="Read more" url="'.get_the_permalink().'" size="medium" bg_color="#FF5C00" text_color="#FFFFFF" align="left" target="_self"]'); ?>
<?php
