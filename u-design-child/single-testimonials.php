...
if ( $portfolio_categories_array != "" && post_is_in_category_or_descendants( $portfolio_categories_array ) ) :
    ...
    if($lang_cat=="en")
    	$category_slug = get_category_by_slug('testimonials');
    else
    	$category_slug = get_category_by_slug('kundberattelser');

    if($lang_cat=="en")
    	$testimonial_name = "testimonials";
    else
    	$testimonial_name = "kundberattelser";
...
<?php		if (have_posts()) :
			...
			<div class="testimonial_panel">
				...
				<a href="<?php echo site_url($category_slug->slug);?>"><span
					class="all_posts alignleft">ALLA KUNDBERÄTTELSER</span></a><!-- English text "ALL TESTIMONIALS" --><span
				...
				<?php else: ?>
					<a href="<?php echo site_url($category_slug->slug);?>"><span
					class="all_posts alignleft">ALL TESTIMONIALS</span></a><!-- English text "ALL TESTIMONIALS" --><span
					class="alignright">
					<?php next_post_link('%link', '<div class="next-button">NEXT</div>', TRUE); ?></span><!-- English text "NEXT" --><span
					class="alignright">
					<?php previous_post_link('%link', '<div class="prev-button">PREVIOUS</div>', TRUE); ?></span><!-- English text "PREVIOUS" -->
				<?php endif; ?>
			</div>
